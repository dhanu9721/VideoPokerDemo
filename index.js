...
function handleOrientationChange() {
  if (
    currentGameOrientation == OrientationTypes.Auto &&
    lastOrientation == OrientationTypes.Portrait &&
    (isIphone || isIPad)
  ) {
    ...
    if (
      ...
      currentGameOrientation == OrientationTypes.Portrait &&
      lastOrientation == OrientationTypes.Landscape
    ) {
      showVerticalOrientationGIF();
    } else {
...
function checkCurrentOrientation(newOrientation) {
  ...
  if (orientationChangeCount == 0) {
    ...
    if (
      newOrientation == OrientationTypes.Landscape &&
      (window.orientation === 90 || window.orientation === -90)
    ) {
      hideOrientationGIF();
      checkScroll();
    } else if (
      newOrientation == OrientationTypes.Portrait &&
...
function showHorizontalOrientationGIF() {
  ...
  if (orientationElement) {
    ...
    hideSwipeGIF();
    orientationElement.style.backgroundImage =
      "url('Plugin/en_orientation.gif')";
...
function showVerticalOrientationGIF() {
  ...
  if (orientationElement) {
    ...
    hideSwipeGIF();
    orientationElement.style.backgroundImage =
      "url('Plugin/en_orientation_v.gif')";
    orientationElement.style.visibility = "visible";
    let maskElement = window.document.getElementById("mask");
    maskElement.style.display = "none";
  }
...
let orientationDetail = {
  detail: { orientation: getGameResolutionTypes(gameCode) },
};
